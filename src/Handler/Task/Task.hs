...
removeTasks taskId documentId = do
  ...
  if taskComplete (entityVal task) then (do
    task <- getEntity404' taskId
    ...
    deleteWhere [TaskTaskGroupId >. taskTaskGroupId (entityVal task)]

    -- Delete all taskgroups that are following this taskgroup for this document, they will be reset
    deleteWhere [TaskGroupDocumentId ==. documentId', TaskGroupId >. taskTaskGroupId (entityVal task)]
